...
const normalizeChannel = (value, fallback = "common") =>
  normalizeChannelKey(value, fallback);
...
const rawXianyuThreshold = parseAmountNumber(
  process.env.XIANYU_ORDER_AMOUNT_THRESHOLD,
);
const XIANYU_ORDER_AMOUNT_THRESHOLD =
  rawXianyuThreshold == null ? 10 : Math.max(0, rawXianyuThreshold);
const ACCOUNT_RECOVERY_WINDOW_DAYS = Math.max(
  1,
  toInt(process.env.ACCOUNT_RECOVERY_WINDOW_DAYS, 30),
);
...
const ORDER_TYPE_SET = new Set([
  ORDER_TYPE_WARRANTY,
  ORDER_TYPE_NO_WARRANTY,
  ORDER_TYPE_ANTI_BAN,
]);
...
const isNoWarrantyOrderType = (value) =>
  normalizeOrderType(value) === ORDER_TYPE_NO_WARRANTY;
const isAntiBanOrderType = (value) =>
  normalizeOrderType(value) === ORDER_TYPE_ANTI_BAN;
...
const STRICT_TODAY_DEFAULT = normalizeStrictToday(
  process.env.REDEEM_ORDER_STRICT_TODAY_DEFAULT,
);
...
const resolveXianyuOrderTypeFromActualPaid = (actualPaid) => {
  ...
  const tierDistance = (value) =>
    Math.min(Math.abs(value - 5), Math.abs(value - 15));
  const normalized =
    tierDistance(asCentToYuan) < tierDistance(asYuan) ? asCentToYuan : asYuan;

  // 可配置：低于阈值按“无质保”，大于等于阈值按“质保”
  return normalized < XIANYU_ORDER_AMOUNT_THRESHOLD
    ? ORDER_TYPE_NO_WARRANTY
    : ORDER_TYPE_WARRANTY;
};
...
const mapCodeRow = (row, channelsByKey) => {
  ...
  const channelName =
    storedChannelName ||
    resolveChannelNameFromRegistry(channelsByKey, channelValue) ||
    channelValue;
  return {
    id: row[0],
    code: row[1],
...
export async function redeemCodeInternal({
  ...
  const normalizedRedeemerUid =
    redeemerUid != null ? String(redeemerUid).trim() : "";
  ...
  if (
    requestedChannelConfig.redeemMode === "linux-do" &&
  ...
  const storedChannel =
    codeRow[6] == null ? "" : String(codeRow[6]).trim().toLowerCase();
  const isStoredCommonChannel =
    storedChannel === "" || storedChannel === "common";
  const reservedForUid = codeRecord.reservedForUid
    ? String(codeRecord.reservedForUid).trim()
    : "";
  const reservedForEntryId = codeRecord.reservedForEntryId;
  const reservedForOrderNo = codeRecord.reservedForOrderNo
    ? String(codeRecord.reservedForOrderNo).trim()
    : "";
  const reservedForOrderEmail = codeRecord.reservedForOrderEmail
    ? normalizeEmail(codeRecord.reservedForOrderEmail)
    : "";
  let resolvedOrderType = normalizeOrderType(orderType || codeRecord.orderType);
  const redeemerIdentifier =
    requestedChannelConfig.redeemMode === "linux-do" && normalizedRedeemerUid
  ...
  const fallbackFromCommonChannelAllowed =
    Boolean(allowCommonChannelFallback) &&
  ...
  if (
    requestedChannelConfig.redeemMode === "linux-do" &&
    reservedForUid &&
    reservedForUid !== normalizedRedeemerUid
  ) {
  ...
  if (reservedForOrderNo) {
    ...
    if (orderRow) {
      ...
      if (
        reservedForOrderEmail &&
        reservedForOrderEmail !== normalizeEmail(normalizedEmail)
      ) {
        throw new RedemptionError(
          403,
          "该兑换码已绑定购买邮箱，请使用下单邮箱兑换",
        );
      }
  ...
  try {
    ...
    if (fallbackFromCommonChannelAllowed && codeChannel !== requestedChannel) {
      const requestedChannelName =
        String(requestedChannelConfig?.name || "").trim() || requestedChannel;
    ...
    const rowsModified =
      typeof db.getRowsModified === "function" ? db.getRowsModified() : null;
    if (rowsModified === 0) {
      throw new RedemptionError(400, "该兑换码已被使用");
    }
  ...
  if (chatgptAccountId && accountToken) {
    ...
    if (!inviteResult.success) {
      ...
      console.log(
        `成功邀请用户 ${normalizedEmail} 加入账号 ${chatgptAccountId}`,
      );
      try {
  ...
  const resolvedUserCount =
    typeof syncedAccount?.userCount === "number"
      ? syncedAccount.userCount
      : typeof syncedUserCount === "number"
        ? syncedUserCount
        : currentUserCount;
  const resolvedInviteCount =
    typeof syncedAccount?.inviteCount === "number"
  ...
  return {
    data: {
      ...
      inviteStatus: inviteResult.success
        ? "邀请已发送"
        : "邀请未发送（需要手动添加）",
      inviteDetails: inviteResult.success
        ? inviteResult.response
        : inviteResult.error,
  ...
  async (req, res) => {
    try {
      ...
      `);
        ...
        return res.json(
          result[0].values.map((row) => mapCodeRow(row, channelsByKey)),
        );
      }
      ...
      const pageSize = Math.min(
        pageSizeMax,
        Math.max(1, toInt(req.query.pageSize, 10)),
      );
      ...
      const codes = (dataResult[0]?.values || []).map((row) =>
        mapCodeRow(row, channelsByKey),
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      return await withLocks(
        [`redemption-code:reinvite:${codeId}`],
        async () => {
          const db = await getDatabase();
          const result = db.exec(
            `
        ...
        `,
          ...
          if (!inviteEmail) {
            return res
              .status(400)
              .json({ error: "兑换用户邮箱缺失，无法重新邀请" });
          }

          if (!accountEmail) {
            return res
              .status(400)
              .json({ error: "该兑换码未绑定账号，无法重新邀请" });
          }
        ...
        `,
          ...
          if (
            ...
            accountResult[0].values.length === 0
          ) {
            return res
              .status(404)
              .json({ error: "所属账号不存在，无法重新邀请" });
          }
          ...
          if (!token || !chatgptAccountId) {
            return res
              .status(400)
              .json({
                error: "所属账号缺少 token 或 chatgpt_account_id，无法重新邀请",
              });
          }

          const inviteResult = await inviteUserToChatGPTTeam(
            inviteEmail,
            {
              token,
              chatgpt_account_id: chatgptAccountId,
  ...
  async (req, res) => {
    ...
    `,
      ...
      const availableSlots =
        totalCapacity - currentUserCount - unusedCodesCount;
      ...
      if (count > availableSlots) {
        console.log(
          `请求生成${count}个兑换码，但账号只有${availableSlots}个可用名额（当前${currentUserCount}人，已有${unusedCodesCount}个未使用兑换码），将只生成${actualCount}个`,
        );
      }
      ...
      const resolvedChannelName =
        String(channelConfig.name || "").trim() || normalizedChannel;
      ...
      res.status(201).json({
        ...
        info:
          count > availableSlots
            ? `由于账号可用名额限制（当前${currentUserCount}人 + ${unusedCodesCount}个未使用兑换码），只生成了${actualCount}个兑换码`
            : undefined,
      });
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      const checkResult = db.exec(
        "SELECT id FROM redemption_codes WHERE id = ?",
        [req.params.id],
      );
...
router.patch(
  ...
  async (req, res) => {
    ...
    `,
      ...
      const updatedCode =
        updatedResult.length > 0 && updatedResult[0].values.length > 0
    ...
    if (normalizedChannel === "linux-do") {
      ...
      if (!uid) {
        return res
          .status(401)
          .json({
            error: "缺少 Linux DO session token",
            code: "LINUXDO_SESSION_REQUIRED",
          });
      }
      redeemerUid = uid;
    }
...
router.post("/recover", async (req, res) => {
  try {
    ...
    if (!normalizedEmail) {
      return res
        .status(400)
        .json({ error: "请输入邮箱地址", message: "请输入邮箱地址" });
    }
    ...
    return await withLocks(
      [`account-recovery:${normalizedEmail}`],
      async () => {
        const db = await getDatabase();
        ...
        const eligibleCandidates = candidates.filter(
          (candidate) => !isNoWarrantyOrderType(candidate.orderType),
        );
        ...
        const refundableCandidates = eligibleCandidates.filter((candidate) => {
          ...
          const creditStatus = String(candidate.creditOrderStatus || "")
            .trim()
            .toLowerCase();
          if (creditStatus === "refunded") return false;

          if (candidate.purchaseOrderRefundedAt) return false;
          const purchaseStatus = String(candidate.purchaseOrderStatus || "")
            .trim()
            .toLowerCase();
        ...
        if (refundableCandidates.length === 0) {
          ...
          const windowEndsAt = buildRecoveryWindowEndsAt(
            firstCandidate.originalRedeemedAt,
          );
          return res.status(403).json({
        ...
        let targetCandidate =
          refundableCandidates.find((candidate) => candidate.currentIsBanned) ||
          null;
        ...
        if (!targetCandidate) {
          for (const candidate of refundableCandidates) {
            ...
            try {
              ...
              const userCount =
                typeof usersData?.total === "number" ? usersData.total : null;
        ...
        if (!targetCandidate) {
          if (unknownSyncError) {
            ...
            const windowEndsAt = buildRecoveryWindowEndsAt(
              candidate.originalRedeemedAt,
            );
            const status = Number(unknownSyncError.statusCode) || 503;
            const message =
              unknownSyncError.error?.message || "账号同步失败，请稍后再试";
          ...
          const accountEmail =
            firstCandidate.currentAccountRecordEmail ||
            firstCandidate.currentAccountEmail;
          const cachedAccess = firstCandidate.currentAccountId
            ? accessCache.get(firstCandidate.currentAccountId)
            : null;
          const userCount =
            cachedAccess?.status === "accessible"
              ? (cachedAccess.userCount ?? null)
              : null;
        ...
        return await withLocks(
          [`account-recovery:original:${originalCodeId}`],
          async () => {
          ...
          `,
            ...
            const completedStatus = completedRow?.[0]
              ? String(completedRow[0]).trim().toLowerCase()
              : "";
            const completedAccountEmail = completedRow?.[1]
              ? normalizeEmail(completedRow[1])
              : "";
            const completedAccountForCheck =
              completedAccountEmail ||
            ...
            `,
              ...
              const currentIsBanned = accountStateRow
                ? Number(accountStateRow[0] || 0) === 1
                : null;
              const currentUserCount = accountStateRow
                ? Number(accountStateRow[1] || 0)
                : null;
              const currentInviteCount = accountStateRow
                ? Number(accountStateRow[2] || 0)
                : null;
            ...
            const codeCreatedWithinDays = Math.max(
              1,
              toInt(recoverySettings?.effective?.codeCreatedWithinDays, 7),
            );
            const requireExpireCoverDeadline = Boolean(
              recoverySettings?.effective?.requireExpireCoverDeadline,
            );
            ...
            for (
              let attempt = 1;
              attempt <= ACCOUNT_RECOVERY_REDEEM_MAX_ATTEMPTS;
              attempt += 1
            ) {
              const selectedRecovery = selectRecoveryCode(db, {
                minExpireMs: requireExpireCoverDeadline
                  ? orderDeadlineMs
                  : Date.now(),
                capacityLimit: 6,
              ...
              const recoveryChannel =
                selectedRecovery.recoveryChannel || "common";
              const recoveryAccountEmail =
                selectedRecovery.recoveryAccountEmail;
              ...
              try {
                ...
                recordAccountRecovery(db, {
                  ...
                  recoveryCode,
                  recoveryAccountEmail:
                    redemptionResult.metadata?.accountEmail ||
                    recoveryAccountEmail,
                  status: "success",
                });
                saveDatabase();
                ...
                const shouldRetry =
                  attempt < ACCOUNT_RECOVERY_REDEEM_MAX_ATTEMPTS &&
                  shouldRetryAccountRecoveryRedeem(error);
            ...
            const errorMessage =
              lastAttemptError?.message || "暂无可用通用渠道补录兑换码";
            const responseMessage =
              "暂无可用通用渠道补录账号，请稍后再试或联系客服";
    ...
    try {
      ...
      if (!config?.authorization) {
        return res
          .status(503)
          .json({
            error: "请先在管理后台配置 Authorization（推荐粘贴 curl 命令）",
          });
      }
      if (!config?.cookies) {
        return res
          .status(503)
          .json({ error: "请先在管理后台配置 Cookie（推荐粘贴 curl 命令）" });
      }
      ...
      await recordXhsSyncResult({
        success: false,
        error: error?.message || "同步失败",
      }).catch(() => {});
      res
        .status(500)
        .json({ error: error?.message || "服务器错误，请稍后再试" });
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await withLocks(
        [`xhs-redeem`, `xhs-order:${normalizedOrderNumber}`],
        async () => {
          const orderRecord = await getXhsOrderByNumber(normalizedOrderNumber);
          if (!orderRecord) {
            return res
              .status(404)
              .json({ error: "未找到对应订单，请稍后再试" });
          }
          ...
          if (String(orderRecord.orderStatus || "").trim() === "已关闭") {
            return res
              .status(403)
              .json({ error: "该订单已完成售后退款（已关闭），无法进行核销" });
          }

          const db = await getDatabase();
          const { byKey: channelsByKey } = await getChannels(db);
          const allowCommonFallback = Boolean(
            channelsByKey.get("xhs")?.allowCommonFallback,
          );
          const now = new Date();
          const fallbackToYesterdayEnabled =
            !strictTodayEnabled && now.getHours() >= 0 && now.getHours() < 8;
          const minAccountExpireAt = formatExpireAtComparable(
            addDays(now, HISTORY_CODE_MIN_ACCOUNT_REMAINING_DAYS),
          );
          ...
          if (!selectedCodeRow && allowCommonFallback) {
            const commonFallback = await withLocks(
              ["redemption-codes:pool:common"],
              async () => {
              ...
              `,
                ...
                `,
                  ...
                  commonCodeRow =
                    commonYesterdayResult?.[0]?.values?.[0] || null;
                }
            ...
            const errorCode =
              allTotal === 0
                ...
                : todayTotal <= 0
                  ? "xhs_no_today_codes"
                  : todayUnused <= 0
                    ? "xhs_today_codes_exhausted"
                    : "xhs_codes_unavailable";

            return res
              .status(503)
              .json({ error: OUT_OF_STOCK_MESSAGE, errorCode });
          }
  ...
  async (req, res) => {
    try {
      ...
      if (!order?.orderId) {
        return res
          .status(502)
          .json({ error: "订单详情解析失败，请确认订单号是否正确" });
      }
      const orderForImport = transformXianyuApiOrderForImport(order);
      if (!orderForImport?.orderId) {
        return res
          .status(502)
          .json({ error: "订单详情解析失败，无法写入数据库" });
      }
      const importResult = await importXianyuOrders(
        orderForImport ? [orderForImport] : [],
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await withLocks(
        [`xianyu-redeem`, `xianyu-order:${normalizedOrderId}`],
        async () => {
          const orderRecord = await getXianyuOrderById(normalizedOrderId);
          if (!orderRecord) {
            return res
          ...
          if (String(orderRecord.orderStatus || "").includes("关闭")) {
            return res
              .status(403)
              .json({ error: "该订单已关闭，无法进行核销" });
          }

          const db = await getDatabase();
          const { byKey: channelsByKey } = await getChannels(db);
          const allowCommonFallback = Boolean(
            channelsByKey.get("xianyu")?.allowCommonFallback,
          );
          const now = new Date();
          const fallbackToYesterdayEnabled =
            !strictTodayEnabled && now.getHours() >= 0 && now.getHours() < 8;
          ...
          const resolvedOrderType = resolveXianyuOrderTypeFromActualPaid(
            orderRecord.actualPaid,
          );
